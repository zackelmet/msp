...
import { faCalendarAlt, faPlus, faTrash, faClock } from "@fortawesome/free-solid-svg-icons";
...
function SchedulePageInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetGroupId || !scheduledDate) { setFormError("Select a target group and date."); return; }
    setSubmitting(true);
    setFormError("");
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/scheduled-tests/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-[#7a9bb5] text-sm mt-1">Schedule automated or manual pentests for your target groups.</p>
        </div>

        <div className="grid lg:grid-cols-5 gap-6">
          <form onSubmit={handleSubmit} className="lg:col-span-2 bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl p-5 space-y-4 h-fit">
            <h2 className="text-sm font-semibold text-white">New Scheduled Test</h2>

            {groups.length === 0 && !loading && (
              <div className="text-xs text-yellow-400/80 bg-yellow-400/5 border border-yellow-400/20 rounded-lg p-3">
                You need to{" "}
                <Link href="/app/targets/new" className="underline">create a target group</Link>{" "}
                first.
              </div>
            )}
            ...
            <div className="space-y-1">
              ...
              <select className={inputCls} value={targetGroupId} onChange={(e) => setTargetGroupId(e.target.value)} required>
                <option value="">Select...</option>
                {groups.map((g) => <option key={g.id} value={g.id}>{g.name}{g.clientName ? ` — ${g.clientName}` : ""}</option>)}
              </select>
            </div>
            ...
            <div className="space-y-1">
              ...
              <input type="date" className={inputCls} value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} required />
            </div>
          ...
          <div className="lg:col-span-3 bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl overflow-hidden h-fit">
            <div className="px-5 py-4 border-b border-[#4590e2]/15">
              <h2 className="text-sm font-semibold text-white">Scheduled Tests</h2>
            </div>
              ...
              <div className="divide-y divide-[#4590e2]/10">
                {tests.map((t) => (
                  <div key={t.id} className="flex items-center gap-3 px-5 py-4">
                    <FontAwesomeIcon icon={faClock} className="w-3.5 h-3.5 text-[#7a9bb5] shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white truncate">{t.targetGroupName || "—"}</p>
                      <p className="text-xs text-[#7a9bb5]">
                        {t.testType} · {t.frequency} · {t.scheduledDate ? new Date(t.scheduledDate).toLocaleDateString() : "—"}
                      </p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full border capitalize shrink-0 ${STATUS_COLORS[t.status] || STATUS_COLORS.pending}`}>
                      {t.status}
                    </span>
                    <button
