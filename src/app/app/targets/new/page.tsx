  ...
  const addAsset = () => {
    ...
    setAssets((prev) => [
      ...prev,
      {
        ...
        label: assetLabel.trim() || assetValue.trim(),
  ...
  return (
    <div className="p-6 lg:p-8 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link
          href="/app/targets"
          className="text-[#7a9bb5] hover:text-white transition-colors"
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Scope notes, exclusions, etc."
              value={notes}
        ...
        <div className="bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl p-5 space-y-4">
          ...
          {assets.length > 0 && (
            <div className="space-y-2">
              {assets.map((a) => (
                <div
                  key={a.id}
                  className="flex items-center gap-3 bg-[#0a141f] rounded-lg px-3 py-2"
                >
                  <span className="text-xs text-[#7a9bb5] w-12 shrink-0">
                    {a.type}
                  </span>
                  ...
                  {a.label !== a.value && (
                    <span className="text-xs text-[#7a9bb5]">{a.label}</span>
                  )}
                  <button
                    type="button"
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            >
              {(["url", "ip", "cidr", "domain", "other"] as const).map((t) => (
                <option key={t} value={t}>
                  {t}
                </option>
              ))}
