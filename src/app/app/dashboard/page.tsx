...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { getFirestore, doc, onSnapshot, collection, query, where, orderBy, limit } =
      ...
      unsubAuth = onAuthStateChanged(auth, async (user) => {
        ...
        try {
          const me = await (await fetch(`/api/auth/isAdmin?uid=${user.uid}`)).json();
  ...
  const tiles = [
    ...
    { label: "Pentests launched", value: launched, icon: faJetFighter, color: "text-purple-400", href: "/app/pentests" },
    { label: "Reports ready", value: completed, icon: faFileLines, color: "text-green-400", href: "/app/pentests" },
  ];
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 space-y-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {loading
            ? [...Array(3)].map((_, i) => (
                <div key={i} className="bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl p-5 animate-pulse h-28" />
              ))
            : tiles.map((t) => (
                <div
                  key={t.label}
                ...
                >
                  <FontAwesomeIcon icon={t.icon} className={`w-4 h-4 ${t.color} mb-3`} />
                  ...
                  {t.cta && (
                    <Link
                      ...
                      className={`mt-3 inline-flex items-center gap-1 text-xs font-semibold ${
                        t.urgent ? "text-[#4590e2]" : "text-[#7a9bb5] hover:text-[#4590e2]"
                      }`}
                    >
                      {t.cta} <FontAwesomeIcon icon={faArrowRight} className="w-2.5 h-2.5" />
                    </Link>
                  )}
                </div>
              ))}
        ...
        {!loading && !allDone && (
          <div className="bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl p-6">
            ...
            <p className="text-xs text-[#7a9bb5] mt-0.5 mb-5">Three steps to your first report.</p>
            <div className="grid sm:grid-cols-3 gap-4">
              {steps.map((s) => (
                ...
                >
                  ...
                  <p className="text-xs text-[#7a9bb5] leading-relaxed">{s.sub}</p>
                  {!s.done && (
                    <span className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-[#4590e2]">
                      {s.cta} <FontAwesomeIcon icon={faArrowRight} className="w-2.5 h-2.5" />
                    </span>
                  )}
                </Link>
              ))}
        ...
        <div className="bg-[#0d1e30] border border-[#4590e2]/15 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#4590e2]/15">
            ...
            {pentests.length > 0 && (
              <Link href="/app/pentests" className="text-xs text-[#4590e2] hover:underline">
                View all
              </Link>
            )}
          </div>
          {loading ? (
            <div className="p-6 space-y-3">
              {[...Array(2)].map((_, i) => (
                <div key={i} className="h-10 bg-[#0a141f] rounded animate-pulse" />
              ))}
            </div>
          ) : pentests.length === 0 ? (
            <p className="px-6 py-5 text-sm text-[#7a9bb5]">
              No pentests yet.{" "}
              {hasCredits ? (
                <Link href="/app/ai-pentest-launch" className="text-[#4590e2] hover:underline">
                  Launch your first &rarr;
                </Link>
              ) : (
                <Link href="/app/buy-credits" className="text-[#4590e2] hover:underline">
            ...
            <div className="divide-y divide-[#4590e2]/10">
              {pentests.slice(0, 6).map((p) => (
                <div key={p.id} className="flex items-center justify-between px-6 py-3.5">
                  <p className="text-sm text-white truncate mr-3">{p.targetUrl || p.id}</p>
                  <span
        ...
        <p className="text-center text-xs text-[#5a7590]">
          ...
          <Link href="/contact-sales" className="text-[#7a9bb5] hover:text-[#4590e2] underline">
            Contact us
          </Link>
          .
        </p>
