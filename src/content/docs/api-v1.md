...
_Status: **Beta** — endpoints and payloads may change. Contact us for early access._

Programmatic API for partners (distributors, resellers/MSPs) to provision org
trees, mint keys, launch pentests, and pull results.
...
- **Auth:** `Authorization: Bearer mspp_live_<key>` (or `mspp_test_<key>` for the
  sandbox — no real scans, no billing). Dashboard sessions use Firebase ID tokens
  against the same handlers.
- **Format:** JSON in/out. UTC ISO-8601 timestamps.
- **Idempotency:** mutating calls accept `Idempotency-Key: <uuid>`; a repeated key
  returns the original result instead of acting twice (safe for automated retries).
- **Scoping:** a key is bound to an org node and may act on that node **and its
  subtree** only.
- **Pagination:** `?limit=` (default 50, max 200) + `?cursor=`; responses include
  `next_cursor`.
- **Errors:** standard HTTP + `{ "error": { "code", "message", "details"? } }`.
  - `401` bad/missing key · `403` out of scope · `402` quota exhausted (hard) ·
    `409` idempotency/state conflict · `422` validation · `429` rate/concurrency limit.

---

## Org tree

A general tree of typed nodes: `platform → distributor → reseller → tenant`. Every
node carries a materialized `path[]` (root→self).
...
A pool lives on the purchasing node (usually the distributor), denominated in
**per-SKU buckets** (`ai_pentest`, `external`, `internal`, `manual`, …). Draw-down
is shared across the subtree; child **caps** are optional ceilings.
...
Mint a scoped key. The full secret is returned **once**; only a SHA-256 hash +
prefix are stored.
```jsonc
// request
...
### `POST /pentests`
Launch. Runs a walk-up entitlement check (tier capability + pool/cap), reserves
units, and enqueues the job.
```jsonc
// request
...
Rollup over the subtree from the append-only usage ledger — the same data
consolidated invoices are computed from.
```jsonc
...
Register endpoints and MSPP POSTs signed events (`X-MSPP-Signature`, HMAC-SHA256,
with retry).
...
**Event types:** `pentest.queued` · `pentest.completed` · `pentest.failed` ·
`report.ready` · `quota.soft_exceeded` · `quota.exhausted`.
```jsonc
// example delivery
